        ...
        return (
            <div className="animate-fade-in-down h-full bg-transparent p-6 md:p-8 rounded-tl-3xl">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
                    ...
                    <button onClick={openCreate} className="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2.5 rounded-md font-bold shadow-lg transition-all flex items-center gap-2 text-sm tracking-widest flex-shrink-0">
                        <span>➕</span> YENİ KURUM EKLE
                    </button>
                </div>

                {/* 🎛️ FİLTRELEME VE ARAMA KONTROL PANELİ */}
                <div className="glass-panel p-4 rounded-xl shadow-lg border border-white/40 mb-8 flex flex-col xl:flex-row gap-4">
                    {/* Arama Çubuğu */}
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full bg-transparent border border-white/40 rounded-lg pl-11 pr-4 py-2.5 text-sm font-semibold focus:glass-panel focus:border-blue-700 outline-none transition-all placeholder:text-slate-400"
                        />
                    </div>
                    ...
                    <div className="flex flex-wrap md:flex-nowrap gap-3">
                        ...
                        <select
                            ...
                            className="w-full md:w-36 bg-transparent border border-white/40 rounded-lg px-3 py-2.5 text-sm font-semibold focus:glass-panel focus:border-blue-700 outline-none transition-all cursor-pointer text-slate-700"
                        >
                        ...
                        <select
                            ...
                            className={`w-full md:w-36 border rounded-lg px-3 py-2.5 text-sm font-semibold outline-none transition-all ${cityFilter === 'ALL' ? 'bg-slate-100 border-slate-100 text-slate-400 cursor-not-allowed' : 'bg-transparent border-white/40 focus:glass-panel focus:border-blue-700 cursor-pointer text-slate-700'}`}
                        >
                ...
                {filteredAndSortedSchools.length === 0 ? (
                    <div className="glass-panel rounded-xl border border-white/40 py-20 text-center shadow-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredAndSortedSchools.map((school) => (
                            <div key={school.id} onClick={() => openDetail(school)} className="glass-panel p-6 rounded-md shadow-lg border border-white/40 hover:border-blue-400 transition-all cursor-pointer group relative overflow-hidden">
                                ...
                                <div className="relative z-10">
                                    <span className={`px-2 py-1 rounded text-[10px] font-bold tracking-tight text-slate-800 uppercase border tracking-widest ${school.schoolType === 'HIGH_SCHOOL' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
    ...
    return (
        <div className="animate-fade-in-right h-full bg-transparent p-6 md:p-8 rounded-tl-3xl">
            <div className="flex items-center gap-4 mb-8 pb-4 border-b border-white/40">
                <button onClick={goToList} className="text-slate-500 hover:text-slate-900 glass-panel border border-slate-300 p-2 rounded-md transition-all shadow-lg font-bold px-4">
                    GERİ DÖN
                </button>
                <div>
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="glass-panel rounded-md shadow-lg border border-white/40 overflow-hidden">
                        <div className="bg-slate-800 p-8 text-white flex items-center gap-6">
                            ...
                            <div>
                                <h3 className="text-3xl font-bold tracking-tight text-slate-800">{selectedSchool.name}</h3>
                        ...
                        <div className="p-6 bg-transparent border-t border-white/40 flex justify-end gap-3">
                            <button onClick={() => handleDelete(selectedSchool.id)} className="glass-panel border-2 border-red-200 text-red-600 hover:bg-red-50 px-6 py-2 rounded-md font-bold text-sm tracking-widest transition-all">
                                SİSTEMDEN SİL
                            </button>
                            <button onClick={openEdit} className="bg-slate-900 hover:bg-blue-700 text-white px-8 py-2 rounded-md font-bold text-sm tracking-widest shadow-lg transition-all">
                                DÜZENLE
                            </button>
                        </div>
                    </div>
            ...
            {(viewMode === 'create' || viewMode === 'edit') && (
                <div className="max-w-4xl mx-auto glass-panel p-8 md:p-10 rounded-md shadow-lg border border-white/40">
                    <form onSubmit={viewMode === 'create' ? handleCreateSubmit : handleUpdateSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select required value={formData.schoolType} onChange={e => setFormData({...formData, schoolType: e.target.value})} className="w-full bg-slate-100 border-2 border-transparent rounded-md px-4 py-3 text-slate-900 font-bold focus:glass-panel focus:border-blue-700 outline-none transition-all cursor-pointer">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-slate-100 mt-6">
                            ...
                            <div>
                                ...
                                <select required value={formData.district} onChange={e => setFormData({...formData, district: e.target.value})} className="w-full bg-slate-100 border-2 border-transparent rounded-md px-4 py-3 text-slate-900 font-bold focus:glass-panel focus:border-blue-700 outline-none transition-all cursor-pointer">
